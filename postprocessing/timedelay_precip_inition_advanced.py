...
EXPID ='lind_p2K' #'test1plus4K' #'lind_p2K' #'test1plus4K'# 'lindp2K'#
DIR = '/nbi/ac/conv1/henneb/results/'
child ='r_int1_10' #'r_int1_10'
v ='twp05_20p' #'twp05_20' # _50_theta1' #_4_theta0'] #,'rwp1'] # ['rwpa2000m05','rwpb2000m05','twp05', 'rwp05','gwp05','cwp05']#,'gwp05']#,'gwp10']#,'cwp']
...
for line in lines:
    ...
    COGx =(float(columns[8]))
    COGy = (float(columns[9]))
    wmax = (float(columns[10]))
    if not rainID in dataparent.keys():
      dataparent[rainID]= RAINCLOUD(rainID)
    # use field for size of cloud for updraft info
    dataparent[rainID].add_info(tist,wmax,precip)
...
# Variante with most overlap
  ...
  if not parentID ==0:  # default if not parent was found
      ...
      # make sure updrafts are already high
      tstart = ttemp
      while dataparent[parentID].inten[ttemp] <= tempinten and dataparent[parentID].size[ttemp] >=5:
          print dataparent[parentID].size[ttemp]
          tstart = ttemp
